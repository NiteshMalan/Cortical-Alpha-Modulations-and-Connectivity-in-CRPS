...
# Usage: python run_preprocess_meg.py PASC001_cb_L_R_good
if len(sys.argv) < 2:
    raise ValueError("Please provide a subject name as the first argument.")

sub = sys.argv[1]
print(f"Processing subject: {sub}")

# --- Determine subject-specific parameters ---
if sub in ['PASC009_rg_R_L_good']:
    num = '3'
else:
    num = '2'
...
# --- Data paths ---
if sub in ["PASC004_tc", "PASC007_ee", "PASC032_pm", "PASC036_ss", "PASC041_aa", 'PASC028_kp_L_R_good', "FASC004_tk_L"]:
    if sub in ['PASC028_kp_L_R_good', "FASC004_tk_L"]:
        data_path = f'/mnt/beegfs/malann/codes/Pain_project/data/PASC_MEG_data/{sub}'
    else:
        data_path = f'/mnt/beegfs/malann/codes/Pain_project/data/PASC_MEG_data/Resting state only/{sub}'
    sub_name = sub[0:7]
elif sub == "PASC001_cb_L_R_good":
    data_path = f'/mnt/beegfs/malann/codes/Pain_project/data/PASC_MEG_data/{sub}/1702806968/'
else:
    data_path = f'/mnt/beegfs/malann/codes/Pain_project/data/PASC_MEG_data/{sub}'
    sub_name = sub[0:7]
